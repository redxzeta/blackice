...
describe('router codex split', () => {
  it('routes non-code chat away from codex models', async () => {
    ...
    const general = chooseChatModel([
      { role: 'user', content: 'explain linux memory pressure in plain terms' },
    ])
    const longform = chooseChatModel([
      { role: 'user', content: `summarize ${'text '.repeat(100)}` },
    ])
