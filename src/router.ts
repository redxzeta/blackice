...
const codeRegex =
  /(typescript|javascript|node\.js|nodejs|python|go|rust|java|c\+\+|debug|bug|stack trace|regex|sql|api|function|class|compile|refactor|test|npm|yarn|pnpm|dockerfile|bash script|shell script|code)/i
const longformRegex =
  /(summarize|summary|rewrite|rephrase|improve writing|draft|long|detailed|condense|extract key points)/i
...
export function chooseChatModel(messages: ChatMessage[]): RouteDecision {
  ...
  if (longformRegex.test(text) && text.length > 300) {
    const nonCodexLongform = isCodexModel(MODEL_LONGFORM)
      ? getObservabilityModel(MODEL_LONGFORM)
      : MODEL_LONGFORM
    return { model: nonCodexLongform, reason: 'longform_summary' }
  }

  const nonCodexDefault = isCodexModel(MODEL_DEFAULT)
    ? getObservabilityModel(MODEL_DEFAULT)
    : MODEL_DEFAULT
