...
type AnalysisPromptRequest = Pick<AnalyzeLogsRequest, 'source' | 'target' | 'hours' | 'maxLines'> | {
  source: 'loki';
  target: string;
  hours: number;
  maxLines: number;
...
async function analyzeFromRawLogs(request: AnalysisPromptRequest, rawLogs: string): Promise<AnalysisResult> {
  if (!rawLogs.trim()) {
...
export function registerLogExplainerRoutes(app: Express): void {
  ...
  app.get('/analyze/logs/metadata', (_req: Request, res: Response) => {
    const status = AnalyzeLogsStatusResponseSchema.parse({
      ...
      limits: {
        ...
        batchConcurrencyMin: BATCH_CONCURRENCY_MIN,
        batchConcurrencyMax: BATCH_CONCURRENCY_MAX,
        loki: getLogCollectorLimits().loki
      },
      targets: {
      ...
      endpoints: {
        ...
        batch: {
          ...
          requestSchema: [
            ...
            {
              source: 'loki',
              query: 'string (optional, raw LogQL)',
              filters: 'record<string,string> (optional, structured selector)',
              contains: 'string (optional)',
              start: 'ISO-8601 datetime (optional)',
              end: 'ISO-8601 datetime (optional)',
              limit: 'number (optional)',
              allowUnscoped: 'boolean (optional)'
            }
          ],
  ...
  app.post('/analyze/logs/batch', async (req: Request, res: Response) => {
    try {
      const parseInput =
        req.body && typeof req.body === 'object' && !Array.isArray(req.body) && !('source' in (req.body as object))
          ? { ...(req.body as Record<string, unknown>), source: 'file' }
          : req.body;

      const parsed = AnalyzeLogsBatchRequestSchema.safeParse(parseInput);
      ...
      if (body.source === 'loki') {
        ...
        try {
          fallbackTarget = buildEffectiveLokiQuery(body);
        } catch {
          // Keep generic fallback label if query construction fails.
        }

        let result: AnalyzeLogsBatchResultOk | AnalyzeLogsBatchResultError;

        try {
          const collected = await collectLokiBatchLogs(body);
          const analyzed = await analyzeFromRawLogs(
            {
              source: 'loki',
