...
describe('http metrics', () => {
  ...
  it('exports request counters, histogram, and inflight gauge in Prometheus format', async () => {
    ...
    expect(output).toContain('blackice_http_requests_total{route="/widgets/:id",method="GET",status="201"} 1')
    expect(output).toContain('blackice_http_request_duration_ms_bucket{route="/widgets/:id",method="GET",le="+Inf"} 1')
    expect(output).toContain('blackice_http_request_duration_ms_count{route="/widgets/:id",method="GET"} 1')
