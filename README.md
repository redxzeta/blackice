...
- `LOG_COLLECTION_TIMEOUT_MS` (default `15000`; timeout in ms for log collection and Loki batch collection)
- `MAX_COMMAND_BYTES` (default `2000000`; cap for collected command output and Loki response bytes)
- `MAX_QUERY_HOURS` (default `168`; max log query lookback window)
- `MAX_LINES_CAP` (default `2000`; cap applied to collected lines)
- `MAX_CONCURRENCY` (default `5`; max allowed batch concurrency)
- `OLLAMA_TIMEOUT_MS` (default `45000`; timeout in ms for log explainer Ollama requests)
- `OLLAMA_RETRY_ATTEMPTS` (default `2`; retry count for log explainer Ollama requests after the first attempt)
- `OLLAMA_RETRY_BACKOFF_MS` (default `1000`; base retry backoff in ms for log explainer Ollama requests)
